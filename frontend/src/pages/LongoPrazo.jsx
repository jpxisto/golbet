...
import { Trophy, Lock, Clock, Users, Zap, ChevronDown, ChevronUp } from 'lucide-react';

// ── Componente de cada Mercado ─────────────────────────────────────────────────
function MercadoCard({ mercado, minhaAposta, onApostar }) {
  const { usuario } = useAuth();
  const aberto = mercado.status === 'aberto';
  const finalizado = !!mercado.resultado;
  const [expandido, setExpandido] = useState(false);

  // Opção escolhida (se o usuário já apostou)
  const jaApostou = !!minhaAposta;
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '14px 14px 0' }}>
        ...
        {/* Aposta existente do usuário */}
        {jaApostou && (
          <div style={{
            marginBottom: 12, padding: '8px 12px', borderRadius: 8,
            background: minhaAposta.status === 'ganhou'
              ? 'rgba(0,194,100,0.1)'
              : minhaAposta.status === 'perdeu'
              ? 'rgba(255,69,69,0.08)'
              : 'rgba(255,208,0,0.07)',
            border: minhaAposta.status === 'ganhou'
            ...
            <span style={{ color: 'var(--texto-sec)' }}>
              Seu palpite: <strong style={{ color: '#FFD000' }}>{minhaAposta.opcao_escolhida}</strong>
              {' · '}R$ {Number(minhaAposta.valor).toFixed(2)}
            </span>
            <span>
              {minhaAposta.status === 'ganhou' && (
                <span style={{ color: '#00C264', fontWeight: 700 }}>+R$ {Number(minhaAposta.premio).toFixed(2)} ✅</span>
              )}
              {minhaAposta.status === 'perdeu' && (
                <span style={{ color: 'var(--vermelho)' }}>❌ Não acertou</span>
              )}
              {minhaAposta.status === 'pendente' && (
        ...
        {expandido && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 14 }}>
            {mercado.opcoes.map(opcao => {
              const eSelecionada = jaApostou && minhaAposta.opcao_escolhida === opcao;
              const eVencedora = finalizado && mercado.resultado === opcao;

              return (
                <button
                  key={opcao}
                  disabled={!aberto || !usuario || jaApostou}
                  onClick={() => aberto && usuario && !jaApostou && onApostar(mercado, opcao)}
                  style={{
                    display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                    padding: '10px 14px', borderRadius: 8, cursor: (!aberto || !usuario || jaApostou) ? 'default' : 'pointer',
...
function ApostaLongoPrazoDrawer({ mercado, opcaoSelecionada, onClose, onSucesso }) {
  ...
  async function confirmarAposta() {
    ...
    try {
      ...
      fetchSaldo();
      addToast(`Aposta de R$ ${valorNum.toFixed(2)} em "${opcaoSelecionada}" registrada! 🎯`, 'sucesso');
      onSucesso();
      onClose();
    } catch (e) {
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div>
            <div style={{ fontWeight: 800, fontSize: 16 }}>Aposta Longo Prazo</div>
...
export default function LongoPrazo() {
  ...
  const [drawer, setDrawer] = useState(null); // { mercado, opcao }
  ...
  // Mapa de aposta por mercado para acesso rápido
  const apostaMap = {};
  minhasApostas.forEach(a => { apostaMap[a.mercado_id] = a; });
      ...
      <div style={{
        ...
        <div>
          ...
          <p style={{ margin: 0, fontSize: 12, color: 'var(--texto-muted)' }}>
            Aposte no campeão do torneio antes da hora
          </p>
        </div>
        {abertos > 0 && (
          <div style={{
      ...
      {mercados.length === 0 ? (
        ...
        mercados.map(mercado => (
          <MercadoCard
            key={mercado.id}
            mercado={mercado}
            minhaAposta={apostaMap[mercado.id]}
            onApostar={(m, opcao) => setDrawer({ mercado: m, opcao })}
          />
        ))
      )}
    </div>
